...
mod tests {
  ...
  // 8 verified ciphertexts for AES-128 with key (10 Rounds of AES)
  const EXPECTED_OUTPUTS: [[u8; 16]; 8] = [
  ...
  // 8 verified ciphertexts for AES-128 with key (10 Rounds of AES)
  const FLAT_EXPECTED_OUTPUTS: [[u8; 64]; 2] = [
    [
      0x39, 0x25, 0x84, 0x1D, 0x02, 0xDC, 0x09, 0xFB, 0xDC, 0x11, 0x85, 0x97, 0x19, 0x6A, 0x0B, 0x32,
      0x97, 0x9E, 0xAD, 0x03, 0x27, 0x10, 0x60, 0x08, 0x7C, 0x29, 0x1D, 0x2E, 0xDC, 0x51, 0x99, 0x8D,
      0x46, 0x3E, 0x34, 0xF5, 0x84, 0x97, 0x74, 0xA7, 0x49, 0x22, 0xD8, 0xFC, 0x1F, 0x42, 0x4F, 0xCA,
  ...
  fn test_aes_ni_c() {
    ...
    // test decode
    let decoded_block = unsafe { decode_aes_ni_c_128(FLAT_KEYS, block, 1) };
    assert_eq!(decoded_block, INPUTS[0]);
  }
